...
function help(){
  ...
  for (let i = 0; i < n; i++) {
    ...
    heights1[i] = parseInt(getRandomValue(1, height));
    bars1.push(document.createElement('div'));
    bars1[i].style.width = `${lineWidth}px`;
    bars1[i].style.height = `${heights1[i]}px`;
    bars1[i].style.transform = `translate(${i * lineWidth + i}px)`;
    bars1[i].style.backgroundColor = 'white';
    bars1[i].className = 'bar';
    container.appendChild(bars1[i]);
    if (n <= 60) {
      barValues1.push(document.createElement('div'));
      barValues1[i].innerHTML = heights1[i];
      barValues1[i].style.marginBottom = `${heights1[i] + 5}px`;
      barValues1[i].style.transform = `translate(${i * lineWidth + i}px)`;
      barValues1[i].className = 'barValue';
      container.appendChild(barValues1[i]);
    }
  }
}
